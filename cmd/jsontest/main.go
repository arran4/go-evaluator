...
func usage() {
	...
	fmt.Fprintln(os.Stderr, "Evaluate a JSON document against the expression. Reads from stdin when no files are specified. Exits with status 1 if the expression does not match.")
	flag.PrintDefaults()
}
